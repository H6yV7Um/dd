...
function smarty_compiler_widget($arrParams,  $smarty){
    ...
    $strClass = isset($arrParams['class']) ? $arrParams['class'] : 'null';
    $is_rend = isset($arrParams['is_rend']) ? 0 : 1;
    $textarea_style = isset($arrParams['textarea_style']) ? $arrParams['textarea_style'] : 'null';
    //是否通过api接口获取widget内容
    $fetch_widget = isset($arrParams['fetch_widget']) ? $arrParams['fetch_widget'] : 'null';
    ...
    unset($arrParams['is_rend']);
    unset($arrParams['textarea_style']);
    unset($arrParams['fetch_widget']);
    ...
    if($strName){
        $strCode .= '$hit = FISPagelet::start(' . $strPageletId . ',' . $strMode . ',' .$strGroup.','.$strClass.','.$is_rend.','.$textarea_style.','.$fetch_widget.');';

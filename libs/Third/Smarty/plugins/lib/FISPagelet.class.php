...
class FISPagelet {
    ...
    static public function insertPageletGroup($html) {
        ...
        foreach (self::$_pagelet_group as $group => $groupInfo) {
            ...
            $url = $groupInfo['url'];
            $replace[] = '<textarea class="'.$class.'" style="'.$style.'">BigPipe.asyncLoad([{id: "'.
                implode('"},{id:"', $ids)
            .'"}],"'.$url.'");</textarea>';
        }

...
class FISResource {
    ...
    //国际化定制
    public static function setCdn($url) {
        self::$cdn = $url;
    }

    //国际化定制
    public static function getCdn($type = null) {
        if (self::$cdn && $type != 'tpl') {
            return self::$cdn;
        }
        return '';//直接返回默认的js，css,img的cdn。http://static01.cliponyu.com
    }
    ...
    public static function addStatic($uri, $type) {
        if (self::$isInnerWidget) {
            self::$arrWidgetStatic[$type][] = self::getCdn($type) . $uri;
        } else {
            self::$arrStaticCollection[$type][] = self::getCdn($type) . $uri;
        }
    }
    ...
    public static function getFramework() {
        return self::getCdn("js").self::$framework;
    }

...
class OAuth2Client_YJP
{
	...
	public function authenticate( $code )
	{
	   ...
	   // echo $this->token_url;die("kkkkk");
		$this->curl_authenticate_method  = "POST";
		$temp = base64_encode("{$this->client_id}:{$this->client_secret}");
	...
	*/
	...
	{
		Hybrid_Logger::info( "Enter OAuth2Client_YJP::request( $url )" );
		Hybrid_Logger::debug( "OAuth2Client_YJP::request(). dump request params: ", serialize( $params ) );
		...
		Hybrid_Logger::debug( "OAuth2Client_YJP::request(). dump request info: ", serialize( curl_getinfo($ch) ) );
		Hybrid_Logger::debug( "OAuth2Client_YJP::request(). dump request result: ", serialize( $response ) );

...
class OAuth1ClientKas{
	...
	*/
	function get($url, $parameters = array(),$isToken=true)
	{
		return $this->api($url, 'GET', $parameters,$isToken=true); 
	} 
	...
	* Make signed request  
	*/ 
	function signedRequest( $url, $method, $parameters ,$isToken=true)
	{
		if($isToken){
			...
			$request = OAuthRequest::from_consumer_and_token($this->consumer, null, $method, $url, $parameters);
			$request->sign_request($this->sha1_method, $this->consumer, null);
		}
		switch ($method) {
	...
	*/ 
	function request( $url, $method, $postfields = null, $auth_header = null )
	{
        ...
        curl_setopt( $ci, CURLOPT_HTTPHEADER, array( 'Content-Type: text/json', $auth_header ) );

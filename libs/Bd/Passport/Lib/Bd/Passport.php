...
class Bd_Passport {
	...
	public static  $arrAllFields = array ("username","userid","loginpass","securemobil","secureemail",
    ...
    **/
	...
	public static function oauthThirdpass($params) {
		$ins = Bd_Passport_Thirdpass::getInstance();
		$ret = $ins->oatuhlogin($params);
		if ($ins->isError()) {
			self::$_errno	= $ins->getCode();
			self::$_errmsg	= $ins->getMessage();
			return false;
     ...
     * @date	2011/06/06 14:20:35
    **/
	public static function B($bduss = null, $incomplete_user = 0) {
//        var_dump($_COOKIE);exit;
		...
		$ssnData = self::getData($bduss, $incomplete_user);
     ...
     * @return	用户不存在或交互失败返回false，
     * @note 
     * @author	liuzhenhua
     * @date	2011/02/17 14:20:35
    **/
    public static function userLogout($bduss){
		$ins = Bd_Passport_Session::getInstance();
		$ret = $ins->logout($bduss);
		if ($ins->isError()) {
			self::$_errno	= $ins->getCode();
			self::$_errmsg	= $ins->getMessage();
			return false;
     ...
     * @date	2011/02/17 14:20:35
    **/
    public static function getData($bduss, $incomplete_user = 0){
		$ins = Bd_Passport_Session::getInstance();
		$ret = $ins->getData($bduss, $incomplete_user);
		if ($ins->isError()) {
			self::$_errno	= $ins->getCode();
			self::$_errmsg	= $ins->getMessage();
    ...
    **/
     ...
     * @date 2011/02/17 14:20:35
    **/
    public static function authGetData($bduss, $stoken, $incomplete_user = 0){
		$ins = Bd_Passport_Session::getInstance();
		$ret = $ins->authGetData($bduss , $stoken, $incomplete_user);
		if ($ins->isError()) {
			self::$_errno	= $ins->getCode();
			self::$_errmsg	= $ins->getMessage();
			return false;
    ...
    **/
	...
	public static function getBDUSSBySSID($ssid) {
		...
		if ($ins->isError()) {
        	...
        	return isset($ret['BDUSS']) ? $ret['BDUSS'] : false;
    	}
	}
     ...
     * @date    2011/06/06 14:20:35
     **/
    public static function checkUserLogin($bduss = null, $incomplete_user = 0) {
//        var_dump($_COOKIE);exit;
        if ($bduss === null) {
            if (isset($_COOKIE['BDUSS'])) {
                $bduss = $_COOKIE['BDUSS'];
		...
		if (!(Bd_Passport_Util::decodeBduss($bduss))) {
			return false;
		}
		$ssnData = self::getData($bduss, $incomplete_user);
		if (is_array($ssnData) && $ssnData['uid'] > 0) {

...
class Bd_Passport_Util {
    ...
    public static function getBdsToken($src, $key, $srcex, &$dst) {
        ...
        if (!(is_string($src) && is_string($key))) {
            $dst = sprintf("the fisrt two args should be string type:%d,%d!", intval(is_string($src)), intval(is_string($key)));
            return false;
        }
    ...
    public static function checkBdsToken($token, $src, $key, $srcex) {
        ...
        if (!(is_string($src) && is_string($key) && is_string($token))) {
            $error = sprintf("the fisrt three args should be string type:%d,%d,%d!", intval(is_string($token)), intval(is_string($src)), intval(is_string($key)));
            return $error;
        }
    ...
    public static function int2ip($num) {
        $tmp = (double) $num;
        return sprintf('%u.%u.%u.%u', $tmp & 0xFF, (($tmp >> 8) & 0xFF), (($tmp >> 16) & 0xFF), (($tmp >> 24) & 0xFF));
    }
     ...
     * 默认保留前两位
     * 
     * ex.  test123@baidu.com
     *      te*****@baidu.com
     * 
     * @param string $email 邮箱地址
     * @param int $start 开头保留位数 默认为 2
     * @param int $end 结尾保留位数 默认为 0
     * @return string 隐藏部分地址的邮箱
     */
    public static function hideEmail($email, $start = 2, $end = 1) {
        $at = strpos($email, '@');
        if ($at <= ($start + $end)) {
            return $email;
        }
        ...
        $prefix = substr($email, 0, $start);
        $middle = '...';
        $suffix = substr($email, $end);
     ...
     * @param int $start 开头保留位数 默认为 3 
     * @param int $end 结尾保留位数 默认为 2
     * @return string 隐藏部分数字的手机号码
     */
    public static function hideMobile($mobile, $start = 3, $end = 2) {
        $len = strlen($mobile);

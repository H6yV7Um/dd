...
//            var_dump($arrInput);exit;
	...
	public function login_v2($intUid, $strUname, $gdata, $gmask, $pdata, $pmask, $exptime=0, $email=null, $mobil=null,$displayname= null) {
		$arrInput = array(
			'cmd'		=> self::CMD_SSN2_LOGIN,
			'apid'		=> self::$_apid, 
			'clientip'	=> ip2long(CLIENT_IP),
			'uid'		=> intval($intUid),
			'uname'		=> strval($strUname),
			'displayname'		=> strval($displayname),
			'(raw)gdata'	=> $gdata,
			'(raw)gmask'	=> $gmask,
			'(raw)pdata'	=> $pdata,
			'(raw)pmask'	=> $pmask,
	...
	public function getData($strBduss,$incomplete_user = 0) {
		$arrInput = array(
		...
		Bd_Passport_Log::debug("session get data params : ".json_encode($arrInput), 0);
		return self::_____(__FUNCTION__ , $arrInput);
	}
	...
	public function authGetData($strBduss , $stoken , $incomplete_user = 0) {

...
class Bd_Rpc_Http extends Bd_Rpc_Abstract
{
    ...
    protected $_intConnectTimeout = 500;
    
    protected $_intTimeout = 2000;
    ...
    {
        $this->_init();
        Bingo_Log::debug('arrInput='.json_encode($arrInput),"dal");
        ...
        while ($intRetry -- ) {
        	...
        	Bingo_Log::debug("Http request Server: ".json_encode($this->_arrNowServer),"dal");
        	if (! $arrNowServer) {
        	    //connect error, 
        	    Bingo_Log::warning('get connect server failure!server='.$this->_strServerName,"dal");

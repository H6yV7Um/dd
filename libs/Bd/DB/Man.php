...
interface Bd_DB_IHostSelector
{
    public function select(Bd_DB_Man $dbman, $key = NULL);
}
...
class Bd_DB_Man
{
    ...
    public function __construct(
        ...
        $dbname,                     // 'mydb'
        Bd_DB_IHostSelector $hostSelector, // new Bd_DB_RandSelector()
        Bd_DB_IStatusMan $statusMan = NULL // new Bd_DB_StatusManFile('tmp')
    )
    {
        ...
        if($statusMan !== NULL)
        {
            ...
            foreach($this->allHosts as $key => &$host)
            {
                ...
                {
                    // filter failed host
                    if(($status['last_failed_time'] + Bd_DB_Man::$RETRY_INTERVAL) > $now)
                    {
//                        echo "filter: {$host[0]}:{$host[1]}\n";
    ...
    public function getDB($key = NULL, $getNew = false, $replace = true)
    {
        ...
        $db = new Bd_DB(Bd_DB_Man::$ENABLE_PROFILING);
        if(Bd_DB_Man::$CONN_TIMEOUT > 0)
        {
            $db->setConnectTimeOut(Bd_DB_Man::$CONN_TIMEOUT);
        }
        ...
        while(true)
        {
            ...
            // try count exceeded
            if(++$try_count == Bd_DB_Man::$MAX_TRY_COUNT)
            {
                return false;
            }
        }

...
class Global_Fastdfs_Storage extends Global_Fastdfs_Base {
     ...
     * @return array
     */
	public function uploadFileByContent($index,$fileContent,$ext) {

        if(strlen($ext) > self::FILE_EXT_NAME_MAX_LEN)
        {
           	$this->_errstr = "file ext too long.";
            $this->_errno = 0;
        ...
        if($ext === '') {
        	$this->_errstr = "file ext is empty.";
            $this->_errno = 0;
            Bingo_Log::warning($this->_errstr,"dal");
            return FALSE;
        }
        ...
        $requestBodyLength = 1 + self::PROTO_PKG_LEN_SIZE + 
            self::FILE_EXT_NAME_MAX_LEN + $fileSize;

        $requestHeader = self::buildHeader(11, $requestBodyLength);
        $requestBody   = pack('C', $index) . self::packU64($fileSize) . self::padding($ext, self::FILE_EXT_NAME_MAX_LEN);
        $this->send($requestHeader . $requestBody);

        //stream_copy_to_stream($fp, $this->_sock, $fileSize);
        $this->send($fileContent,$fileSize);
        
        $responseHeader = $this->read(self::HEADER_LENGTH);
        $responseInfo   = self::parseHeader($responseHeader);

        if($responseInfo['status'] !== 0){
        	$this->_errstr = "something wrong with uplode file";
            $this->_errno = $responseInfo['status'];
            Bingo_Log::warning($this->_errstr,"dal");
            return FALSE;
        }
...
/**
     ...
     */
    public function uploadSlaveFileByContent($fileContent, $masterFilePath, $prefix_name, $ext = '') {
    ...
    if(strlen($ext) > self::FILE_EXT_NAME_MAX_LEN)
        ...
        $requestBodyLength = 16 + self::FILE_PREFIX_MAX_LEN + 
            self::FILE_EXT_NAME_MAX_LEN + $masterFilePathLen + $fileSize;

        $requestHeader = self::buildHeader(21, $requestBodyLength);
        $requestBody   = pack('x4N', $masterFilePathLen) . self::packU64($fileSize) . self::padding($prefix_name, self::FILE_PREFIX_MAX_LEN);
        $requestBody  .= self::padding($ext, self::FILE_EXT_NAME_MAX_LEN) . $masterFilePath;
        ...
        if($responseInfo['status'] !== 0) {
			...
			Bingo_Log::warning( $msg . " status:" .$responseInfo['status'],'dal');
			return false;
        }
...
//                $msg, 
//                $responseInfo['status']);
			$this->_errstr = $msg . " status:" .$responseInfo['status'];
            $this->_errno = 0;
            Bingo_Log::warning( $this->_errstr,'dal');
			return false;
        }
    ...
    public function appendFile($content, $appenderFilePath) {
        ...
        $requestBodyLength                =  (2 *  self::PROTO_PKG_LEN_SIZE) + $appenderFilePathLength + $content_length;
    ...
    public function modifyFile($filename, $filePath, $offset = 0) {
        ...
        $fileSize        = filesize($filename);
        $requestBodyLength       =  (3 *  self::PROTO_PKG_LEN_SIZE) + $filePathLength + $fileSize;
    ...
    public function deleteFile($groupName, $filePath) {
        ...
        $requestBodyLength = strlen($filePath) + self::GROUP_NAME_MAX_LEN;
        $requestHeader      = self::buildHeader(12, $requestBodyLength);        
        $requestBody        = self::padding($groupName, self::GROUP_NAME_MAX_LEN) . $filePath;
     ...
     */
    public function setFileMetaData($groupName, $filePath, array $meta_data, $flag =  self::OVERWRITE_METADATA) {

        $meta_data        = self::buildMetaData($meta_data);
        $meta_data_length = strlen($meta_data);
        $filePathLength = strlen($filePath);
        $flag = $flag === self::OVERWRITE_METADATA ? 'O' : 'M';

        $requestBodyLength = (self::PROTO_PKG_LEN_SIZE * 2) + 1 + $meta_data_length + $filePathLength + self::GROUP_NAME_MAX_LEN;

        $requestHeader      = self::buildHeader(13, $requestBodyLength);

        $requestBody =  self::packU64($filePathLength) . self::packU64($meta_data_length);
        $requestBody .= $flag . self::padding($groupName, self::GROUP_NAME_MAX_LEN) . $filePath . $meta_data;
    ...
    public function readFile($groupName, $filePath, $offset = 0, $length = 0) {
        ...
        $filePathLength = strlen($filePath);
        $requestBodyLength  = (self::PROTO_PKG_LEN_SIZE * 2) + $filePathLength + self::GROUP_NAME_MAX_LEN;
        
        $requestHeader       = self::buildHeader(14, $requestBodyLength);

        $requestBody         = self::packU64($offset) . self::packU64($length) . self::padding($groupName, self::GROUP_NAME_MAX_LEN);
    ...
    public function getFileInfo($groupName, $filePath) {
        ...
        $fileSize     = self::unpackU64(substr($responseBody, 0, self::PROTO_PKG_LEN_SIZE));
        $timestamp     = self::unpackU64(substr($responseBody, self::PROTO_PKG_LEN_SIZE, self::PROTO_PKG_LEN_SIZE));
        list(,,$crc32) = unpack('N2', substr($responseBody, 2 * self::PROTO_PKG_LEN_SIZE, self::PROTO_PKG_LEN_SIZE));
